...
keys: Dict[str, Key] = {}
for i in range(4):
    keys[f"ed25519_{i}"] = Key.from_securesystemslib_key(
        {
            "keytype": "ed25519",
            "scheme": "ed25519",
            "keyid": keyids[i],
...
def generate_all_files(
    ...
    i = 0
    for md in [md_root, md_timestamp, md_snapshot, md_targets]:
        assert isinstance(md, Metadata)
        key_info = copy.deepcopy((keys[f"ed25519_{i}"].to_securesystemslib_key()))
        key_info["keyval"]["private"] = private_values[0]
        signer = SSlibSigner(key_info)
        md.sign(signer)
