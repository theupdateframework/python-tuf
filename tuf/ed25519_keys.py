...
def generate_public_and_private():
  ...
  <Purpose> 
    ...
    and private keys returned conform to 'tuf.tufformats.ED25519PULIC_SCHEMA' and
    'tuf.ssl_crypto.formats.ED25519SEED_SCHEMA', respectively, and have the form:
    ...
    >>> tuf.ssl_crypto.formats.ED25519PUBLIC_SCHEMA.matches(public)
    True
    >>> tuf.ssl_crypto.formats.ED25519SEED_SCHEMA.matches(private)
    True
  ...
  <Returns>
    A (public, private) tuple that conform to 'tuf.ssl_crypto.formats.ED25519PUBLIC_SCHEMA'
    and 'tuf.ssl_crypto.formats.ED25519SEED_SCHEMA', respectively.
  """
...
def create_signature(public_key, private_key, data):
  ...
  <Purpose>
    ...
    to 'tuf.ssl_crypto.formats.ED25519SIGNATURE_SCHEMA', and has the form:
  ...
  # 'tuf.ssl_crypto.formats.ED25519PUBLIC_SCHEMA', which must have length 32 bytes.
  # Raise 'tuf.ssl_commons.exceptions.FormatError' if the check fails.
  tuf.ssl_crypto.formats.ED25519PUBLIC_SCHEMA.check_match(public_key)

  # Is 'private_key' properly formatted?
  tuf.ssl_crypto.formats.ED25519SEED_SCHEMA.check_match(private_key)
  ...
  # The private and public keys have been validated above by 'tuf.tufformats' and
...
def verify_signature(public_key, method, signature, data, use_pynacl=False):
  ...
  tuf.ssl_crypto.formats.NAME_SCHEMA.check_match(method)
  
  # Is 'signature' properly formatted?
  tuf.ssl_crypto.formats.ED25519SIGNATURE_SCHEMA.check_match(signature)
  
  # Is 'use_pynacl' properly formatted?
  tuf.ssl_crypto.formats.BOOLEAN_SCHEMA.check_match(use_pynacl)
  ...
  if method in _SUPPORTED_ED25519_SIGNING_METHODS:
    ...
    # Verify 'ed25519' signature with the pure Python implementation. 
      ...
      try:
        tuf.ssl_crypto._vendor.ed25519.ed25519.checkvalid(signature, data, public)
        valid_signature = True
