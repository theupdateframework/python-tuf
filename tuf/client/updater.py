...
class Updater(object):
  ...
  def download_target(self, target, destination_directory, repo_name=None):
    ...
    # attempts at acquiring the target fail. Each NoWorkingMirror exception,
    # in turn, contains the error generated by every mirror of that repository.
    # The structure is:
    #   exceptions_from_all_delegations = [
    #       // Exceptions from first delegation - e.g. to Repo1
    #       [ tuf.NoWorkingMirrorError(     # Exception from trying Repo1
    #           <the individual exceptions from each mirror for Repo1>) ],
    #
    #       # Exceptions from second delegation, tried after first fails.
    #       # This delegation is a multi-repo delegation to Repo2 and Repo3.
    #       [ tuf.NoWorkingMirrorError(...),  # Exception from trying Repo2
    #         tuf.NoWorkingMirrorError(...) ] # Exception from trying Repo3
    #
    exceptions_from_all_delegations = []
    #exceptions_from_this_delegation = []
    ...
    for repo_list in relevant_pinnings:
      ...
      for repo_name in repo_list:
        ...
        try:
          ...
          exceptions_from_this_delegation.append(e)
      ...
      # We land here if all attempts to download the target from a repository
      # listed in the current delegation failed.
      exceptions_from_all_delegations.append(exceptions_from_this_delegation)
    ...
    all_mirror_errors = {}
    for delegation_errors in exceptions_from_all_delegations:
      for e in delegation_errors:
        for mirror_url, mirror_error in six.iteritems(e.mirror_errors):
          ...
          # We need to add some prefix relating to the delegation to prevent
          # this.
