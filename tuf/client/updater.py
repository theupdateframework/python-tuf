...
class Updater(object):
  ...
  def _ensure_not_expired(self, metadata_role):
    ...
    # an exception.  'expires' is in 'tuf.formats.UNIX_TIMESTAMP_SCHEMA' format,
    # the number of seconds since the epoch (January 1, 1970.)  Compare it
    ...
    # current time (i.e., a local time).  In contrast, the logger message
    # contains the 'expires' unix timestamp.
    datetime_object = tuf.formats.unix_timestamp_to_datetime(expires)
    local_time = datetime_object.ctime()
    if expires < current_time:
      message = 'Metadata '+repr(rolepath)+' expired on '+repr(expires)+'.'
      logger.error(message)

      # Expired metadata message generated by
      # tuf.ExpiredMetadataError (defined in __init__.py)
      raise tuf.ExpiredMetadataError(local_time)
