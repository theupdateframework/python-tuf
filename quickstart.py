...
def try_file_entry():
  prompt_message = \
    '\nIf you have an input file, enter its name. Otherwise, just press Enter: '
  for attempt in range(MAX_INPUT_ATTEMPTS):
    input_file = _prompt(prompt_message)
    if input_file == '':
      return None
    else:
      try:
        ...
        message = 'Invalid file name entered.'
        logger.error(message)
        print(message)
        data = None
  return data

def get_timeout(input_date):
  try:
...
def get_role_info(keystore_directory, role_data = None):
  role_info = {}
  for role in ['root', 'targets', 'release', 'timestamp']:
    ...
    if role_data != None:
      try:
        role_threshold = role_data[role]['threshold']
    ...
    if role_data != None:
      for password in role_data[role]['passwords']:
      ...
      try:
        i = 0
        while True:
          message = 'Enter a password for '+repr(role)+' ('+str(i+1)+'), or leave blank to continue: '
        ...
        message = 'Warning: Not enough ' + role + ' passwords entered to match the threshold.'
        logger.warning(message)
        print(message)
  return role_info
...
def build_repository(project_directory):
  ...
  file_data = try_file_entry()
  role_info = {}
  if (file_data is not None):
    try:
      timeout = get_timeout(file_data['expiration_date'])
      if timeout < 1:
        raise ValueError
      role_info = get_role_info(keystore_directory, file_data['roles'])
      if role_info is None:
        raise ValueError
    except ValueError, e:
      message = 'File data is invalid or incomplete. Reverting to manual entry.'
      logger.error(message)
      print(message)
      file_data = None
  ...
  if (file_data is None):
    ...
    for attempt in range(MAX_INPUT_ATTEMPTS):
      try:
        ...
        timeout = get_timeout(input_date)
        if timeout < 1:
          raise ValueError
        break
      except ValueError, e:
    ...
    role_info = get_role_info(keystore_directory)
    if role_info is None:
      raise tuf.RepositoryError('Could not build the keystore\n')
